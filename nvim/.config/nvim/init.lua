...
require('lazy').setup({
  ...
  {
    -- Terminal Plugin + Telescope Terminal Viewer
    ...
    config = function()
      ...
      local ok_tt, term_mod = pcall(require, 'toggleterm.terminal')
      if not ok_tt then
        return
      end
      ...
      local function next_term_count()
        local all = term_mod.get_all()
        local max = 0
        for _, t in pairs(all) do
          if t.count and t.count > max then
            max = t.count
          end
        end
        return max + 1
      ...
      -- Terminal Viewer (Telescope Picker)
      local function terminal_picker()
        ...
        for _, t in pairs(term_mod.get_all()) do
          local label = string.format('#%d  %-9s  %s', t.count or 0, t.direction or 'float', t.cmd or (vim.env.SHELL or 'shell'))
          table.insert(entries, { display = label, ordinal = label, term = t })
        end
        ...
        pickers
          .new({}, {
            ...
            attach_mappings = function(bufnr, map)
              ...
              map('i', '<C-x>', close_selected)
              map('n', '<C-x>', close_selected)
              return true
            end,
          })
          :find()
      ...
      vim.keymap.set('n', '<leader>st', terminal_picker, { desc = '[S]earch [T]erminal viewer' })

      -- Toggle the current terminal (last opened or focused)
      vim.keymap.set('n', '<leader>tt', function()
        local term_mod = require 'toggleterm.terminal'
        local term

        -- If we're inside a ToggleTerm buffer, use its buffer-local terminal id
        if vim.bo.buftype == 'terminal' and vim.b.toggle_number then
          term = term_mod.get(vim.b.toggle_number)
        end

        -- Otherwise try "current/last" (may be nil)
        term = term or term_mod.get(0)

        -- Fallback: first existing terminal, if any
        if not term then
          for _, t in pairs(term_mod.get_all()) do
            term = t
            break
  ...
  },
  {
    ...
    event = 'InsertEnter',
    init = function()
      -- Don't let Copilot grab <Tab> (blink.cmp uses it)
      vim.g.copilot_no_tab_map = true
      vim.g.copilot_assume_mapped = true

      -- Enable/disable by filetype (tweak to taste)
    ...
    keys = {
      -- Accept suggestion (ghost text)
      { '<C-g>', 'copilot#Accept("<CR>")', mode = 'i', expr = true, replace_keycodes = false, desc = 'Copilot Accept' },
      -- Cycle suggestions
      { '<M-]>', 'copilot#Next()', mode = 'i', expr = true, desc = 'Copilot Next' },
      { '<M-[>', 'copilot#Previous()', mode = 'i', expr = true, desc = 'Copilot Prev' },
      -- Dismiss
      { '<C-]>', 'copilot#Dismiss()', mode = 'i', expr = true, desc = 'Copilot Dismiss' },
      -- (Optional) Manually prompt a suggestion
      -- { "<C-\\>", 'copilot#Suggest()', mode = "i", expr = true, desc = "Copilot Suggest" },
    },
  },
